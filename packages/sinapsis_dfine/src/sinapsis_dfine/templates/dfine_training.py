...
class DFINETraining(DFINEBase):
    ...
    attributes: DFINETrainingAttributes
    ...
    def _initialize_model(self) -> None:
        ...
        if self.attributes.training_mode == "from-scratch":
            ...
            self.model = DFineForObjectDetection(config=config).to(self.device)  # ty: ignore[invalid-argument-type]
        else:
    ...
    def _load_datasets(self) -> Tuple[Any, Any]:
        ...
        """
        val_split_size = self.attributes.validation_split_size
        ...
        if "validation" not in dataset and "test" not in dataset:
            self.logger.warning(f"No 'validation' or 'test' split found. Splitting with ratio {val_split_size}.")
            if val_split_size <= 0.0:
                raise ValueError("'validation_split_size' must be > 0 when no validation set is provided.")

            split = dataset["train"].train_test_split(test_size=val_split_size)  # ty: ignore[unresolved-attribute]
            return split["train"], split["test"]
        else:
            train_dataset = dataset["train"]
            validation_dataset = dataset.get("validation", dataset.get("test"))  # ty: ignore[unresolved-attribute]
